...
namespace Grit.Net.Common.DAL
{
    ...
    public class MSSqlGeneralDAL<T> : IGeneralDAL<T> where T : TEntity, new()
    {
        /// <summary>
        /// 类名
        /// </summary>
        ...
        private SqlParameter ParamFilterValid = null;
        private SqlParameter ParamFilterInValid = null;
        private string ValidColumnName = null;

        public MSSqlGeneralDAL()
        {
            GetPropertyInfo();
        }
        #region 过滤属性和字段名信息
            ...
            foreach (PropertyInfo pi in properties)
            {
                ...
                if (idAttri == null)
                {
                    ...
                    if (ValidColumnName == null)
                    {
                        ...
                        if (validAttri != null)
                        {
                            ParamFilterValid = new SqlParameter(string.Format("@{0}", pi.Name), validAttri.Available);
                            ParamFilterInValid = new SqlParameter(string.Format("@{0}", pi.Name), validAttri.Invalid);
        ...
        public T Get(object IdValue)
        {
            ...
            parms.Add(new SqlParameter("@" + IdColomnName, IdValue));
        ...
        {
            IList<T> result = null;
            string cmdText = string.Format("SELECT {0} FROM {1} {2}", GetColumnText, TableName, ClassName);
        ...
        public IList<T> GetList(PageInfo page)
        {
            ...
            DataSet ds = DataHelper.ExecuteDataSet(cmdText.ToString(), CommandType.Text, null);
        ...
        public bool Insert(T t)
        {
            ...
            for (int i = 0; i < tableProperties.Length; i++)
            {
                ...
                parms.Add(new SqlParameter("@" + pi.Name, pi.GetValue(t)));
            }
        ...
        public bool Update(T t)
        {
            ...
            {
                pi = tableProperties[i];
                if (pi.Name == IdColomnName && thisIdGenerateType != IdGenerateType.Assign)
                {
                    continue;
                }
                if (pi.Name != IdProperty.Name)
                    parms.Add(new SqlParameter("@" + pi.Name, pi.GetValue(t)));
            }
        ...
        public bool Delete(T t)
        {
            ...
            IDataParameter[] parms = new SqlParameter[] { new SqlParameter("@" + IdColomnName, value) };
        ...
        {
            IDataParameter[] parms = new SqlParameter[] { new SqlParameter("@" + IdColomnName, idValue) };
            string cmdText = string.Format("delete from {0} where {1}=@{1}", TableName, IdColomnName);
            return DataHelper.ExecuteNonQuery(cmdText, CommandType.Text, parms) > 0;
        }
        public bool InsertAndGet(T t)
        {
            ...
            for (int i = 0; i < tableProperties.Length; i++)
            {
                ...
                parms.Add(new SqlParameter("@" + pi.Name, getter(t, null)));
            }
            ...
            {
                SetValueDelegate setter = FastMethodFactory.CreatePropertySetter(pi);
                setter(t, obj);
                return true;
            }
            return false;
        ...
        /// <returns></returns>
        public bool IsExist(string PropertyName, object PropertyValue)
        {
        ...
        private void PropertyToColumnName(string strWhere, StringBuilder sb, object[] parmsValue, IDataParameter[] parms, ref int parmIndex)
        {
            ...
            if (indexFlag == -1)
            {
                ...
                if (columnValue.Equals("?"))
                {
                    ...
                    parms[parmIndex] = new SqlParameter("@" + parmIndex, parmsValue[parmIndex]);
                    parmIndex++;
                }
                else
                {
        ...
        public bool IsExist(string PropertyName, object PropertyValue, object IdValue)
        {
            ...
            IList<SqlParameter> parms = new List<SqlParameter>();
            parms.Add(new SqlParameter("@" + colunmName, PropertyValue));
        ...
        {
            IDataParameter[] parms = new SqlParameter[] { new SqlParameter("@" + IdColomnName, IdValue), ParamFilterInValid };
