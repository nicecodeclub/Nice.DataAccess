...
{
    public class MSSqlQueryDAL<T> : IQueryDAL<T> where T : TModel, new()
    {
        /// <summary>
        /// 类型属性集合
        /// </summary>
        ...
        private void FilterQueryText(ref string cmdText)
        {
            ...
            for (int i = 0; i < entityNames.Length; i++)
            {
                ...
                sb.Append(DataFactory.EntityAndTables[entityName]);
        ...
        {
            StringBuilder sb = new StringBuilder();
            parms = new SqlParameter[parmsValue.Length];
            cmdText = cmdText.ToUpper();
            int index = cmdText.IndexOf(flagWhere);
            string strSelect = cmdText.Remove(index);
            int indexTable = cmdText.IndexOf(flagFrom);
        ...
        private void PropertyToColumnName(string strWhere, StringBuilder sb)
        {
            ...
            sb.Append(' ');
            sb.Append(DataFactory.PropertyAndColumns[nameAndColumn]);
        ...
        private void PropertyToColumnName(string strWhere, StringBuilder sb, object[] parmsValue, IDataParameter[] parms, ref int parmIndex)
        {
            ...
            sb.Append(' ');
            sb.Append(DataFactory.PropertyAndColumns[columnName]);
        ...
        public IList<T> GetListBySQL(string cmdText, PageInfo page)
        {
            ...
            if (page != null)
            {
                sb.AppendFormat(" ORDER BY {0} {1} OFFSET {2} ROW FETCH NEXT {3} ROW ONLY ", page.OrderColName, page.OrderStr, page.StartIndex, page.PageSize);
    ...
    public class MSSqlQueryDAL : IQueryDAL
    {
        #region  执行SQL
        /// <summary>
        /// 执行SQL
        ...
        public int ExecuteNonQuery(string[] cmdText, object[][] parmsValue)
        {
            ...
            if (parmsValue != null)
            {
                dbps = new SqlParameter[cmdText.Length][];
                for (int i = 0; i < cmdText.Length; i++)
                {
                    text = cmdText[i];
